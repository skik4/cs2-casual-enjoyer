...
class FriendsRenderer {
  ...
  static renderFriendsList(friends, joinStates = {}) {
    ...
    const showTutorialMockFriend = appStateManager.getState(
      "showTutorialMockFriend"
    );
    ...
    if (showTutorialMockFriend) {
      ...
      friendsToRender = friendsToRender.filter(
        (f) => f.steamid !== TUTORIAL_MOCK_FRIEND.steamid
      );
    ...
    if (
      ...
      friendsToRender[0].steamid === TUTORIAL_MOCK_FRIEND.steamid
    ) {
    ...
    this.updateFriendsListIncremental(
      friendsContainer,
      filteredFriends,
      joinStates
    );
  ...
  static updateFriendsListIncremental(container, friends, joinStates = {}) {
    ...
    for (const friend of friends) {
      ...
      const avatarUrl =
        friend.avatarfull || friend.avatar || friend.avatarmedium || "";
      ...
      if (existingElement) {
        ...
        const nextSibling = lastInsertedElement
          ? lastInsertedElement.nextElementSibling
        ...
        // Create new element
        const newElement = this.createFriendElement(
          friend,
          joinState,
          isMissing,
          avatarUrl
  ...
  static createFriendElement(friend, joinState, isMissing, avatarUrl) {
    ...
    wrapper.innerHTML = this.renderFriendItem(
      friend,
      joinState,
      isMissing,
      avatarUrl
  ...
  static updateFriendElement(element, friend, joinState, isMissing, avatarUrl) {
    ...
    const statusText = isMissing
      ? t("friends.temporarilyNotSupported")
      : friend.status;
  ...
  static updateFriendControls(element, friendId, joinState, isMissing) {
    ...
    // Update join button
    if (joinButton) {
      const newButtonText = isActive ? t("common.cancel") : t("common.join");
      ...
      const shouldBeDisabled =
        joinState && joinState.status === STATUS_TYPES.JOINED;
