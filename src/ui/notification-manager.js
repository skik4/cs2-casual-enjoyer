...
class NotificationManager {
  ...
  static showUpdateError(steamId = "") {
    const currentSteamId =
      steamId || DOMUtils.getElementById("steam-id")?.value.trim() || "";
   ...
   * @private
   */
  static _cleanupCS2LaunchNotification(
    launchBtn,
    closeBtn,
    handleLaunch,
    handleClose
    ...
    logger.debug(
      "NotificationManager",
      "Starting CS2 launch notification cleanup"
    );
    ...
    if (launchBtn && handleLaunch) {
      ...
      logger.debug(
        "NotificationManager",
        "Removed launch button event listener"
      );
    }
    ...
    logger.info(
      "NotificationManager",
      "CS2 launch notification cleanup completed"
    );
  }
  ...
  static showCS2LaunchNotification(friendId, cs2Manager) {
    logger.info("NotificationManager", "Starting CS2 launch notification", {
      friendId,
      hasCs2Manager: !!cs2Manager,
    });
    ...
    return new Promise((resolve) => {
      ...
      if (!overlay) {
        logger.error(
          "NotificationManager",
          "CS2 notification overlay not found"
        );
        resolve(false);
        return;
      }
      ...
      if (!launchBtn || !closeBtn) {
        logger.error(
          "NotificationManager",
          "CS2 notification buttons not found after template creation"
        );
        resolve(false);
        return;
      }
      ...
      const handleLaunch = async () => {
        ...
        try {
          const success = await cs2Manager.launchAndWaitForLobby((status) => {
            logger.debug("NotificationManager", "CS2 launch progress", {
              status,
            });
          });
          ...
          logger.error(
            "NotificationManager",
            "Error during CS2 launch process",
            { error: error.message }
          );
          if (!state.isResolved) {
            state.isResolved = true;
      ...
      const handleClose = () => {
        ...
        logger.info(
          "NotificationManager",
          "Close button clicked during CS2 launch process"
        );
  ...
  static hideCS2LaunchNotification() {
    ...
    const overlay = DOMUtils.getElementById("cs2-launch-notification");
    if (overlay) {
      logger.debug("NotificationManager", "Hiding CS2 notification overlay", {
      ...
      logger.warn(
        "NotificationManager",
        "CS2 notification overlay not found in DOM"
      );
    }
  }
}
