...
export class TutorialUIManager {
  ...
  setupModalEventListeners(callbacks, isFirstStep, isLastStep) {
    ...
    const prevBtn = this.modal.querySelector(
      ".tutorial-nav-buttons .tutorial-btn-secondary"
    );
    const skipBtn = this.modal.querySelector(
      ".tutorial-controls > .tutorial-btn-secondary"
    );
  ...
  positionModal(targetElement, forcedPosition = null) {
      ...
      switch (forcedPosition) {
        ...
        case "center":
          ...
          requestAnimationFrame(() => {
            ...
            const centerLeft =
              centerViewportWidth / 2 - centerModalRect.width / 2;
            const centerTop =
              centerViewportHeight / 2 - centerModalRect.height / 2;
    ...
    const fitsBelow =
      top + modalRect.height <= viewportHeight - horizontalPadding;
    ...
    if (!fitsBelow) {
      ...
      if (fitsAbove) {
        ...
        const fitsRight =
          leftRight + modalRect.width <= viewportWidth - horizontalPadding;
        ...
        if (fitsRight) {
          ...
          top = Math.max(
            ...
            Math.min(
              centerY,
              viewportHeight - modalRect.height - horizontalPadding
            )
          );
        } else {
