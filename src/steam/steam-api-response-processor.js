...
class SteamAPIResponseProcessor {
  ...
  static processFriendsListResponse(rawData, isToken) {
    logger.debug(
      "SteamAPIResponseProcessor",
      "Processing friends list response",
      { isToken }
    );
    ...
    if (friendIds.length === 0) {
      logger.warn(
        "SteamAPIResponseProcessor",
        "No confirmed friends found after filtering"
      );
    ...
    logger.info(
      "SteamAPIResponseProcessor",
      `Processed ${friendIds.length} confirmed friends`,
      {
  ...
  static processPlayerSummariesResponse(rawData, chunkIndex = 1) {
    logger.debug(
      "SteamAPIResponseProcessor",
      `Processing player summaries response chunk ${chunkIndex}`
    );
    ...
    logger.info(
      "SteamAPIResponseProcessor",
      `Chunk ${chunkIndex}: processed ${players.length} player summaries`
    );

    // Debug logging for processed player data
    logger.debug(
      "SteamAPIResponseProcessor",
      `Player summaries chunk ${chunkIndex} details`,
      {
   ...
   */
  static async processFriendsStatusesResponse(
    rawData,
    avatarsCache = {},
    getPlayerSummariesCallback = null
  ) {
    logger.debug(
      "SteamAPIResponseProcessor",
      "Processing friends statuses response"
    );
    ...
    const cs2Players = accounts.filter(
      (acc) => acc.private_data?.game_id === "730"
    );
    logger.info(
      "SteamAPIResponseProcessor",
      `Found ${cs2Players.length} friends playing CS2`
    );

    // Pre-filter for supported players to only load avatars for them
    const supportedPlayerSteamIds =
      this._extractSupportedPlayerSteamIds(cs2Players);
    logger.info(
      "SteamAPIResponseProcessor",
      `Found ${supportedPlayerSteamIds.length} friends in supported modes (casual/deathmatch)`
    );
  ...
  static processConnectInfoResponse(rawData) {
    logger.debug(
      "SteamAPIResponseProcessor",
      "Processing connect info response"
    );
    ...
    if (!accounts.length) {
      logger.debug(
        "SteamAPIResponseProcessor",
        "No accounts in connect info response"
      );
      return null;
    }
    const priv = accounts[0].private_data || {};
    if (!SteamAPIUtils.isPlayerInCS2(priv)) {
      logger.debug("SteamAPIResponseProcessor", "Player not in CS2", {
        game_id: priv.game_id,
      });
      return null;
    }

    const richPresence = SteamAPIUtils.parseRichPresence(
      priv.rich_presence_kv || ""
    );
    if (SteamAPIUtils.isInSupportedMode(richPresence)) {
      logger.debug(
        "SteamAPIResponseProcessor",
        "Found connect info for supported game mode player",
        {
    ...
    logger.debug(
      "SteamAPIResponseProcessor",
      "Player not in joinable supported game mode",
      {
        game_mode: richPresence.game_mode,
        game_state: richPresence.game_state,
      }
  ...
  static processGameServerSteamIdResponse(rawData) {
    logger.debug(
      "SteamAPIResponseProcessor",
      "Processing game server Steam ID response"
    );
    ...
    if (!accounts.length) {
      logger.debug(
        "SteamAPIResponseProcessor",
        "No accounts in game server response"
      );
      return null;
    }
    ...
    const serverId =
      richPresence.game_server_steam_id || priv.game_server_steam_id || null;

    logger.debug(
      "SteamAPIResponseProcessor",
      "Extracted game server Steam ID",
      {
   ...
   */
  static processPlayerCS2StatusResponse(
    rawData,
    steam_id,
    requireLobby = false
  ) {
    logger.debug(
      "SteamAPIResponseProcessor",
      "Processing player CS2 status response",
      {
        steam_id,
        requireLobby,
        hasData: !!rawData,
        ...
        hasAccounts: !!(
          rawData &&
          rawData.response &&
          rawData.response.accounts
        ),
        accountsLength:
          rawData && rawData.response && rawData.response.accounts
            ? rawData.response.accounts.length
            : 0,
    ...
    if (
      ...
      logger.warn(
        "SteamAPIResponseProcessor",
        `No player data found for ${steam_id}`,
        {
          dataExists: !!rawData,
    ...
    if (!isInCS2) {
      logger.info(
        "SteamAPIResponseProcessor",
        `Player ${steam_id} CS2 status: not playing`,
        {
          steam_id,
          game_id: priv.game_id,
          isInCS2: false,
    ...
    if (!requireLobby) {
      logger.info(
        "SteamAPIResponseProcessor",
        `Player ${steam_id} CS2 status: playing (lobby check not required)`,
        {
          steam_id,
          game_id: priv.game_id,
          isInCS2: true,
    ...
    logger.info(
      "SteamAPIResponseProcessor",
      `Player ${steam_id} CS2 status: ${isInCS2 ? "playing" : "not playing"}, in lobby: ${isInLobby}`,
      {
        steam_id,
        game_id: priv.game_id,
        game_state: richPresence.game_state,
  ...
  /**
   * Process user game mode response to check if user is in Competitive or Premier mode
   * @param {Object} rawData - Raw response from Steam API
   * @param {string} steam_id - Steam ID being checked
   * @returns {boolean} - Whether the user is in Competitive or Premier mode
   */
  static processUserGameModeResponse(rawData, steam_id) {
    logger.debug(
      "SteamAPIResponseProcessor",
      "Processing user game mode response",
      {
        steam_id,
        hasResponse: !!rawData,
        hasAccounts: !!(
    ...
    if (
      ...
      !rawData.response ||
      !rawData.response.accounts ||
      rawData.response.accounts.length === 0
    ) {
      logger.info(
        "SteamAPIResponseProcessor",
        `No account data found for game mode check ${steam_id}`,
        {
          steam_id,
          hasResponse: !!rawData,
          hasAccounts: !!(
    ...
    if (!isInCS2) {
      logger.info(
        "SteamAPIResponseProcessor",
        `Player ${steam_id} not in CS2, game mode check not applicable`,
        {
          steam_id,
          game_id: priv.game_id,
          isInCS2: false,
    ...
    // Check if user is in Competitive or Premier mode
    const isInCompetitiveOrPremier =
      SteamAPIUtils.isPlayerInCompetitiveOrPremier(priv);

    logger.info(
      "SteamAPIResponseProcessor",
      `Player ${steam_id} game mode check result`,
      {
        steam_id,
        isInCS2: true,
        isInCompetitiveOrPremier,
        game_extra: priv.game_extra,
        rich_presence: priv.rich_presence,
      }
    );

    return isInCompetitiveOrPremier;
  }
   ...
   * @returns {Promise<import('../shared/types.js').Friend[]>} - Array of processed friend objects
   */
  static async processFriendsStatusesWithAvatars(
    rawData,
    avatarsCache = {},
    apiClient = null
  ) {
    logger.debug(
      "SteamAPIResponseProcessor",
      "Processing friends statuses response with avatar fetching"
    );

    // Create callback for fetching additional avatars if API client is provided
    const getPlayerSummariesCallback = apiClient
      ? async (steamids) =>
          await apiClient.getPlayerSummaries(steamids, apiClient._lastUsedAuth)
      : null;
   ...
   * @private
   */
  static async _processAvatarsForSupportedPlayers(
    supportedPlayerSteamIds,
    avatarsCache,
    getPlayerSummariesCallback
  ) {
    ...
    if (supportedPlayerSteamIds.length > 0) {
      ...
      const missingAvatars = SteamAPIUtils.filterMissingAvatars(
        supportedPlayerSteamIds,
        avatarsCache
      );
      ...
      if (missingAvatars.length > 0 && getPlayerSummariesCallback) {
        logger.info(
          "SteamAPIResponseProcessor",
          `Fetching avatars for ${missingAvatars.length} supported players`
        );
        const fetchedAvatars = await getPlayerSummariesCallback(missingAvatars);
        avatarMap = { ...avatarsCache, ...fetchedAvatars };
        logger.info(
          "SteamAPIResponseProcessor",
          `Retrieved ${Object.keys(fetchedAvatars).length} new avatars`
        );
      } else {
        avatarMap = avatarsCache;
        logger.info(
          "SteamAPIResponseProcessor",
          `Using cached avatars for all ${supportedPlayerSteamIds.length} supported players`
        );
      }
    }
  ...
  static _logAllFriendsBreakdown(accounts) {
    logger.debug("SteamAPIResponseProcessor", "All friends breakdown", {
      allFriends: accounts.map((acc) =>
        SteamAPIUtils.createPlayerDebugInfo(acc)
      ),
    });
  }
  ...
  static _logFriendsBreakdown(mapped) {
    ...
    logger.info(
      "SteamAPIResponseProcessor",
      `Found ${supportedFriends.length} friends in supported modes (casual/deathmatch), ${joinableFriends.length} joinable`
    );
    ...
    logger.debug("SteamAPIResponseProcessor", "Non-supported CS2 friends", {
      nonSupportedFriends: nonSupportedFriends.map((f) => ({
        ...
        reason: !f.game_mode
          ? "No game mode"
          : !["casual", "deathmatch"].includes(f.game_mode)
            ? `Playing ${f.game_mode}`
            : ["", null, "lobby"].includes(f.game_state)
              ? `In ${f.game_state || "lobby"}`
              : "Other reason",
      })),
    });
